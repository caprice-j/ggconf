...
```{r, echo=FALSE, eval=FALSE, fig.height=9, fig.width=10}
suppressPackageStartupMessages(library(dplyr))
gg <- ggplot(mtcars[1:20, ] %>% tibble::rownames_to_column() %>% 
             mutate(car_name = rowname, maker = gsub(" .*", "", car_name) ) ) + 
      #geom_label(aes(mpg, qsec, label = substr(car_name, 1, 13), color=maker),
...
```{r, echo=FALSE, eval=FALSE, fig.height=9, fig.width=10}
gg +
ggplot2::theme(text = ggplot2::element_text(face="bold", size=24, family="Times New Roman"), panel.background = ggplot2::element_rect(fill="white"), legend.box.margin = ggplot2::margin(0.2,0.2,0.2,0.2,"cm"), legend.box.background = ggplot2::element_rect(colour="black"), legend.key = ggplot2::element_rect(fill="white"), legend.position = ("bottom"), legend.text = ggplot2::element_text(size=rel(0.8)), legend.title = ggplot2::element_text(family="Consolas", colour="royalblue"), axis.title = ggplot2::element_text(family="Consolas", colour="royalblue"), axis.title.y = ggplot2::element_text(angle=0, vjust=0.5), axis.text = ggplot2::element_text(size=rel(1.1)), axis.line = ggplot2::element_line(arrow=arrow(type="open",angle=20), size=2), axis.ticks = ggplot2::element_line(size=1), axis.ticks.length = grid::unit(0.5,"cm"), plot.subtitle = ggplot2::element_text(face="plain", hjust=1), plot.margin = ggplot2::margin(0.3,0.3,0.3,0.1,"inch"))
```
